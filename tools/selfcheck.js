...
const BAN = [
  ...
  // 2026-08-30 用戶指示：香港商報唔用得（建制報，同已禁嘅大公文匯同類）
  [/hkcd\.com\.hk/i, "香港商報"], [/香港商報/, "香港商報"],
];
const bad = [];
const warn = [];
G.forEach(k => {
...
if (!Array.isArray(d.track) || d.track.length < 3) bad.push(`事件追蹤只有 ${(d.track || []).length} 條`);
  ...
  // 連續多版無新進展 → ⚠（唔會 fail）。2026-08-31 起：20 版轉低頻、40 版可除名（有已知後續除外）
  const edsList = [];
  ...
  // 2026-08-31 用戶定案：門檻改做 20 版轉低頻／40 版除名，所以回望窗要開到 45 版（原本只有 5 版，n 永遠去唔到 20）
const past = ci > 0 ? edsList.slice(Math.max(0, ci - 45), ci).reverse() : [];
  ...
  d.track.forEach(r => {
    ...
    // ── 2026-08-31 用戶定案（原話：「20版無消息改爲低頻跟蹤, 40版之後除名, 有已知後續除外」）──
    // 舊制：連續 4 版就出 ⚠ 叫你問用戶除唔除名 → 太密，而且預設方向錯（08-16／08-23 已定「⚠ 出現預設諗轉低頻」）
    // 新制：20 版 → 直接轉低頻（唔使問）；40 版 → 可以除名，但**有已知未來節點（開庭／聆訊／報告發表／正式開通）一律唔准除名**，改續低頻
    if (LOW.has(r[0])) return;
    if (n >= 40) warn.push(`「${r[0]}」已經連續 ${n} 版無新進展（≥40）→ 按 2026-08-31 用戶定案可以除名，加入 tools/retired.json。⚠ 但凡有已知後續節點（開庭／聆訊／報告發表／正式開通嘅實日子）一律唔准除名，改為續低頻`);
    else if (n >= 20) warn.push(`「${r[0]}」已經連續 ${n} 版無新進展（≥20）→ 按 2026-08-31 用戶定案直接轉低頻跟蹤，加入 tools/lowfreq.json，唔使問用戶、唔准除名`);
  });
}
